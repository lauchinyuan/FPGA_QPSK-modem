...
依照表1中的式子，将成形滤波后的I、Q两路数据与分别与余弦、正弦载波相乘，将基带信号调制到中频，并将相乘后两路信号叠加，即可得到QPSK调制信号，本设计中乘法器使用vivado提供的乘法器IP核实现。
...
首先通过MATLAB跑通QPSK调制解调的基本流程，方便确定一些基本参数（如滤波器阶数等方案），MATLAB仿真的相关结果如图所示。

完成QPSK调制解调的基本流程的实现后，编写了testbench文件对本设计的各个模块进行了仿真，以下分模块进行分析。
